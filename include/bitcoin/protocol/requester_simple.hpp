...
#ifndef LIBBITCOIN_PROTOCOL_REQUESTER_SIMPLE_HPP
#define LIBBITCOIN_PROTOCOL_REQUESTER_SIMPLE_HPP
...
class BCP_API requester_simple {
public:

    requester_simple(zmq::context& context);

    requester_simple(zmq::context& context, const config::endpoint& address);

    requester_simple(const requester_simple&) = delete;
    void operator=(const requester_simple&) = delete;
